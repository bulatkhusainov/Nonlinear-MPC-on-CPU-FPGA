			...
			if (packet_type==1) //reset IP
					...
					else if (packet_type==3) //write data DDR
					{
						...
						switch (packet_internal_ID)
						{
							case 0: //init_in
							if (DEBUG)
									printf("write init_in\n\r");

							if (FLOAT_FIX_INIT_IN==1) {
								for (i=0; i<ETH_PACKET_LENGTH-2; i++)
								{
									tmp_float=*(float*)&inputvec[i];
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,INIT_IN_FRACTIONLENGTH));
								}
								memcpy(init_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(init_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
							
							case 1: //sc_in_in
							if (DEBUG)
									printf("write sc_in_in\n\r");

							if (FLOAT_FIX_SC_IN_IN==1) {
								for (i=0; i<ETH_PACKET_LENGTH-2; i++)
								{
									tmp_float=*(float*)&inputvec[i];
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,SC_IN_IN_FRACTIONLENGTH));
								}
								memcpy(sc_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(sc_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
							...
							if (FLOAT_FIX_BLOCK_IN==1) {
								...
								{
									tmp_float=*(float*)&inputvec[i];
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,BLOCK_IN_FRACTIONLENGTH));
								}
								memcpy(block_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(block_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
							...
							if (FLOAT_FIX_OUT_BLOCK_IN==1) {
								...
								{
									tmp_float=*(float*)&inputvec[i];
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,OUT_BLOCK_IN_FRACTIONLENGTH));
								}
								memcpy(out_block_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(out_block_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
							
							case 4: //v_in_in
							if (DEBUG)
									printf("write v_in_in\n\r");

							if (FLOAT_FIX_V_IN_IN==1) {
								for (i=0; i<ETH_PACKET_LENGTH-2; i++)
								{
									tmp_float=*(float*)&inputvec[i];
									inputvec_fix[i]=(int32_t)(tmp_float*pow(2,V_IN_IN_FRACTIONLENGTH));
								}
								memcpy(v_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec_fix,(ETH_PACKET_LENGTH-2)*4);
							} else { //floating-point
								memcpy(v_in_in_ptr_ddr+(ETH_PACKET_LENGTH-2)*packet_internal_ID_offset,inputvec,(ETH_PACKET_LENGTH-2)*4);
							}
							break;
					...
					else if (packet_type==4) //read data from DDR
					{
						...
						switch (packet_internal_ID)
						{
							case 0: //v_out_in
							if (DEBUG)
								printf("read v_out_out\n\r");
							memcpy(outvec_fix,v_out_out_ptr_ddr+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset,(ETH_PACKET_LENGTH_RECV-2)*4);
							for (i=0; i<ETH_PACKET_LENGTH_RECV-2; i++)
							{
								if (FLOAT_FIX_V_OUT_OUT==1) { //fixed-point
									tmp_float=((float)outvec_fix[i])/pow(2,V_OUT_OUT_FRACTIONLENGTH);
									outvec[i]=*(Xint32*)&tmp_float;
								} else { //floating point
							...
							case 1: //sc_out_in
							if (DEBUG)
								printf("read sc_out_out\n\r");
							memcpy(outvec_fix,sc_out_out_ptr_ddr+(ETH_PACKET_LENGTH_RECV-2)*packet_internal_ID_offset,(ETH_PACKET_LENGTH_RECV-2)*4);
							for (i=0; i<ETH_PACKET_LENGTH_RECV-2; i++)
							{
								if (FLOAT_FIX_SC_OUT_OUT==1) { //fixed-point
									tmp_float=((float)outvec_fix[i])/pow(2,SC_OUT_OUT_FRACTIONLENGTH);
									outvec[i]=*(Xint32*)&tmp_float;
								} else { //floating point
									outvec[i]=outvec_fix[i];
								}
