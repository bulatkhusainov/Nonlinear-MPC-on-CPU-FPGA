...
void mv_mult_prescaled_HW(part_vector *y_out,part_matrix *block,float out_block[(N+1)*n_states],part_vector *x_in)
{
	...
	int row_block_sched[28] = {0,7,8,2,1,10,3,6,0,5,8,2,1,10,4,6,0,5,9,8,1,7,2,3,4,0,9,8,};
	int col_block_sched[28] = {9,5,4,10,8,3,10,4,8,9,2,8,7,2,8,0,6,7,5,1,1,1,2,3,6,0,0,0,};
	int num_block_sched[28] = {3,14,13,9,6,11,11,12,2,15,8,8,5,9,13,1,1,14,15,6,4,5,7,10,12,0,3,2,};
	...
	{
		y_out[i_offset1+row_term_block_sched[j]] += block[i_offset2 + num_term_block_sched[j]]*x_in->vec[i_offset1+col_term_block_sched[j]];
	}
}
