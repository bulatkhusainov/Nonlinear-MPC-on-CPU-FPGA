...
void prescaler(float blocks[N*nnz_block_tril + nnz_term_block_tril],float b[n_all_theta+n_all_nu],float x_current[n_all_theta+n_all_nu])
{
	...
	for(i = 0; i < N; i++)
	{
		...
		for(j = 0; j < n_states; j++) M[j] = 1;
		local_ptr1 += n_states;
		for(j = 0; j < n_node_theta-n_states; j++) M[j] = 0;
		local_ptr1 += (n_node_theta-n_states);
		for(j = 0; j < n_states; j++) M[j] = 1;
		local_ptr1 += n_states;
		for(j = 0; j < (n_node_eq-n_states); j++) M[j] = 0;
	}
	local_ptr1 = &M[n_states + N*(n_node_theta+n_node_eq)];
	for(j = 0; j < (n_states); j++) M[j] = 1;
	local_ptr1 += n_states;
	for(j = 0; j < (n_term_theta + n_term_eq - n_states); j++) M[j] = 0;
	...
	for(i = 0; i < N; i++)
	{
		...
		for(j = 0; j < N; j++)
		{
			M[i_offset1 + row_block[j]] += fabs(blocks[i_offset2 + num_block[j]])
		}
	}

	// calculate prescaler
	for(i = 0; i < N; i++) M[i] = 1/fsqrt(M[i]);
